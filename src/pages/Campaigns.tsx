...
const Campaigns = () => {
  ...
  const handleEdit = async (campaign: Campaign) => {
    try {
      ...
      const response = await fetch(`http://192.168.29.119:9000/api/v1/campaigns/${campaign.id}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
          'Content-Type': 'application/json'
        },
  ...
  const handleSubmit = async (data: CampaignFormData) => {
    ...
    try {
      ...
      const url = editingCampaign 
        ? `http://192.168.29.119:9000/api/v1/campaigns/${editingCampaign.id}`
        : 'http://192.168.29.119:9000/api/v1/campaigns/';
  ...
  const handleFileUploadSubmit = async (campaignId: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://192.168.29.119:9000/api/v1/campaigns/${campaignId}/upload`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
        },
  ...
  const onCallSubmit = callForm.handleSubmit(async (data: CallFormData) => {
    try {
      ...
      const response = await fetch('http://192.168.29.119:9000/api/v1/calls/', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
          'Content-Type': 'application/json'
