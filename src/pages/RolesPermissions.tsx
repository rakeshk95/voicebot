...
export default function RolesPermissions() {
  ...
  const fetchRoles = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('http://192.168.29.119:9000/api/v1/roles/', {
        headers: {
  ...
  const handleSaveRole = async () => {
    try {
      ...
      const baseUrl = 'http://192.168.29.119:9000/api/v1/roles';
  ...
  const handleDeleteRole = async (role: Role) => {
    try {
      setIsDeleting(true);
      const response = await fetch(`http://192.168.29.119:9000/api/v1/roles/${role.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
          'Content-Type': 'application/json'
