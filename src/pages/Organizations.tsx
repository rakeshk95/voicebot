...
const Organizations = () => {
  ...
  const updateOrganization = async (data: z.infer<typeof organizationSchema>) => {
    ...
    try {
      const response = await fetch(`http://192.168.0.6:8000/api/v1/organizations/${editingOrg.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
  ...
  const deleteOrganization = async (id: string) => {
    ...
    try {
      const response = await fetch(`http://192.168.0.6:8000/api/v1/organizations/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
        },
  ...
  const handleView = async (orgId: string) => {
    setIsActionLoading(true);
    try {
      const response = await fetch(`http://192.168.0.6:8000/api/v1/organizations/${orgId}`, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
        },
